//! [![Build Status](https://github.com/nonconvextech/jzlog/workflows/jzlog/badge.svg?branch=main)](https://github.com/nonconvextech/jzlog/actions)
//! ![License](https://img.shields.io/crates/l/jzlog.svg)
//! [![Latest Version](https://img.shields.io/crates/v/jzlog.svg)](https://crates.io/crates/jzlog)
//! [![jzlog](https://docs.rs/jzlog/badge.svg)](https://docs.rs/jzlog)
//!
...
//!
//! `jzlog` mitigates this bottleneck by sending messages to a dedicated logger
//! thread and computing as little as possible in the main/worker thread.
//!
//! `jzlog` can improve log performance in main/worker thread a few times over. See
...
//! ```
//!
//! Configure and initialize jzlog at the start of your `main` function:
//! ```
//! // jzlog re-export `log`'s macros, so no need to add `log` to dependencies
//! use jzlog::appender::FileAppender;
//! use jzlog::{debug, trace};
...
//! let _guard = jzlog::builder().try_init().unwrap();
//!
//! trace!("Hello world!");
//! debug!("Hello world!");
//! info!("Hello world!");
...
//!     // Set `false` to tell jzlog to discard excessive logs.
...
//!     // write logs in jzlog::appender to "./jzlog-appender.log" instead of "./current.log"
//!     .filter("jzlog::appender", "jzlog-appender", LevelFilter::Error)
//!     .appender("jzlog-appender", FileAppender::new("jzlog-appender.log"))
//!     .try_init()
...
//! ## Default Log Format
//!
//! > 2022-04-08 19:20:48.190+08 **298ms** INFO main [src/jzlog.rs:14] My log
//! > message
//!
...
//! jzlog will limit logs after messages are randomly dropped.
//! ```rust
...
//! ## Custom timestamp format
//!
//! `jzlog` relies on the `time` crate for the formatting of timestamp. To use custom time format,
//! first construct a valid time format description,
//! and then pass it to jzlog builder by `jzlog::time_format(&mut self)`.
//!
//! In case an error occurs when formatting timestamp, `jzlog` will fallback to RFC3339 time format.
//!
//! ### Example
//! ```rust
...
//! )
//! .unwrap();
//! let _guard = jzlog::builder().time_format(format).try_init().unwrap();
...
//! ## Log with interval
//!
//! `jzlog` allows to limit the write frequency for individual log calls.
//!
...
//! for different log calls. Internally, `jzlog` records the last print time by a
...
//!
//! ## Log rotation
//! `jzlog` supports log rotation in local timezone. The available rotation
//! periods are:
//!
...
//!
//! ```rust
//! use jzlog::appender::{FileAppender, Period};
//!
//! let logger = jzlog::builder()
//!     .root(
//!         FileAppender::builder()
//!             .path("./mylog.log")
...
//! when using builder.
//!
//! `jzlog` first finds files generated by `jzlog` and cleans outdated logs by
//! last modified time. `jzlog` find generated logs by filename matched by file
//! stem and added datetime.
//!
//! **ATTENTION**: Any files that matchs the pattern will be deleted.
//!
//! ```rust
//! use jzlog::{appender::{Period, FileAppender, Duration}};
//!
...
//! | ------------------------------------------------- | ------------- | --------------------- | --------------------- |
//! | `jzlog`                                           | static string |   75 ns/iter    | 385 ns/iter    |
//! | `jzlog`                                           | with i32      |   106 ns/iter   | 491 ns/iter    |
    ...
    fn write(
        ...
        if self.limit > 0 {
            ...
            *missed_entry = 0;
			...
			writer.flush()
				.unwrap_or_else(|e| eprintln!("logger flush failed: {}", e));
        }
    }
}
...
/// Shared by jzlog formatter
///
/// To further reduce time spent on log macro calls, jzlog saves required data
/// and later construct log string in log thread.
///
/// `JzLogFormat` defines how to turn an reference to record into a box object,
...
/// use std::fmt::Display;
///
/// use jzlog::JzLogFormat;
/// use log::{Level, Record};
///
/// struct MyFormatter;
/// impl JzLogFormat for MyFormatter {
...
/// }
/// ```
pub trait JzLogFormat: Send + Sync {
    /// turn an reference to record into a box object, which can be sent to log thread
    /// and then formatted into string.
    fn msg(&self, record: &Record) -> Box<dyn Send + Sync + Display>;
}

/// Default jzlog formatter
///
/// The default jzlog format is like:
/// ```text
/// INFO main [examples/jzlog.rs:27] Hello, world!
/// ```
///
/// Since jzlog cannot customize timestamp, the corresponding part is omitted.
/// The actual log output is like:
/// ```text
/// 2022-11-22 17:02:12.574+08 0ms INFO main [examples/jzlog.rs:27] Hello, world!
/// ```
pub struct JzLogFormatter;
impl JzLogFormat for JzLogFormatter {
...
///
/// ```
/// # use jzlog::appender::{FileAppender, Duration, Period};
/// # use log::LevelFilter;
/// let logger = jzlog::builder()
///     // use our own format
///     .format(jzlog::JzLogFormatter)
///     // global max log level
///     .max_log_level(LevelFilter::Info)
///     // define root appender, pass anything that is Write and Send
...
///     // write to "jzlog-appender" appender, with different level filter
///     .filter("jzlog::appender", "jzlog-appender", LevelFilter::Error)
///     // write to root appender, but with different level filter
///     .filter("jzlog", None, LevelFilter::Trace)
///     // write to "jzlog" appender, with default level filter
///     .filter("jzlog::appender::file", "jzlog", None)
///     // ----------  configure additional appender ----------
///     // new appender
///     .appender("jzlog-appender", FileAppender::new("jzlog-appender.log"))
///     // new appender, rotate to new file every Day
///     .appender("jzlog", FileAppender::rotate("jzlog.log", Period::Day))
///     .build()
///     .expect("logger build failed");
/// ```
///
/// # Local timezone
/// For performance reason, `jzlog` only retrieve timezone info once and use this
...
/// Handy function to get jzlog builder
#[inline]
...
impl Builder {
    #[inline]
    /// Create a jzlog builder with default settings:
    /// - global log level: INFO
    /// - root log level: INFO
    /// - default formatter: `JzLogFormatter`
    /// - output to stderr
    ...
    #[inline]
    pub fn format<F: JzLogFormat + 'static>(mut self, format: F) -> Builder {
    ...
    ///
    /// Combine with `Builder::filter()`, jzlog can output log in different module
    ...
    /// jzlog will still log up to INFO.
    #[inline]
                        ...
                        Err(RecvTimeoutError::Timeout) => {
                            if last_flush.elapsed() > Duration::from_millis(1000) {
                                ...
                                for err in flush_errors {
                                    log::warn!("Jzlog flush error: {}", err);
                                }
