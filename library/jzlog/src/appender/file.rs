    ...
    pub fn build(self) -> FileAppender {
        ...
        match (builder.rotate, builder.expire) {
            ...
            (Some(period), Some(expire)) => {
                ...
                if Path::new(&builder.path).exists() {
                let created_time = OffsetDateTime::from(std::fs::metadata(&builder.path).unwrap().created().unwrap());
                if created_time.date() != OffsetDateTime::now_utc().date() {
                    std::fs::rename(&builder.path, FileAppender::file(&builder.path, period, &builder.timezone,created_time)).unwrap();
                }
                }
...
impl FileAppender {
    ...
    ///
    /// ```rust
    /// use jzlog::appender::{Duration, FileAppender, Period};
    /// use jzlog::LogTimezone;
    /// use time::UtcOffset;
    ///
    /// let appender = FileAppender::builder()
    ...
    fn file<T: AsRef<Path>>(path: T, period: Period, timezone: &LogTimezone,time: OffsetDateTime) -> PathBuf {
        let p = path.as_ref();
        let dt = time.to_offset(Self::offset_from_timezone(timezone));
        let ts = match period {
        ...
        let file_name = p
            .file_name()
            .map(|x| format!("{}.{}.0000", x.to_string_lossy(), ts))
...
fn clean_expire_log(path: PathBuf, rotate_period: Period, keep_duration: Duration) -> String {
    ...
    let to_remove = std::fs::read_dir(dir)
        ...
        .filter(|x| {
            ...
            if let Some((stem, time)) = name.rsplit_once(".log.") {
    ...
    fn write(&mut self, record: &[u8]) -> std::io::Result<usize> {
        ...
        }) = &mut self.rotate
        {
            if start.elapsed() > *wait {
                ...
                self.file.flush()?;
                let created_time = OffsetDateTime::from(std::fs::metadata(&self.path).unwrap().created().unwrap());
                std::fs::rename(&self.path, Self::file(&self.path, *period, &self.timezone,created_time))?;
